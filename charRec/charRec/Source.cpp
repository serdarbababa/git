...
#define TIME_SPAN			1*50

void objOrientedExample();
...
void work(){
	...
	vector < vector < float > > *cons= new vector < vector < float > >;
	
	for(int i = 0; i < nodes.size()-1;i++)
	{
		vector< float> layer( nodes.at(i)*nodes.at(i+1),0);
		cons->push_back(layer);
	}
	//mat->push_back(
	getchar();
	...
	for(int i = 0 ; i < nodes.size()-1 ;i++){
		...
		for(int j=0;j<cons->at(i).size();j++){
			int tempy =  j/nodes.at(i);
			int tempx = j % nodes.at(i+1);
			cons->at(i).at(j)=((float)rand())/(RAND_MAX*5)+0.4;
			printf( "w(%d, %d)= %f\t",tempy,tempx, cons->at(i).at(j));
			if(tempx == nodes.at(i+1)-1)
				printf("\n");
		}
		printf("\n");
	}
